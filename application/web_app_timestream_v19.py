        ...
        children=[
            dcc.Tabs(id='tabs', value='graph-tab', children=[
                dcc.Tab(label='Data Plot', value='graph-tab', style={
                    ...
                    "backgroundColor": custom_blue_colors[2],  # Slight background change for active tab
                ...
                "borderRadius": "15px",
                "overflow": "hidden",
                "backgroundColor": "transparent",  # Removed background from the tabs container
                "fontWeight": "bold",
                "border": "none",  # Ensure no border around the tabs container
                "margin": "0",  # Ensure no margin between tabs and content
                "padding": "0"  # No padding around tabs
            }),

            html.Div(id='tabs-content', className='ui padded segment', style={
                "borderRadius": "15px",
                # "backgroundColor": neutral_bg_color,  # Changed to neutral background
                "color": custom_blue_colors[2],
                "padding": "20px",  # Ensures there's no visible gap between elements
                "margin": "0"  # No margin to avoid any separation
            })
        ],
        style={"margin": "20px", "borderRadius": "15px", "backgroundColor": f"{custom_blue_colors[1]}10"}
    )
])
    ...
    if tab == 'graph-tab':
        return html.Div([
            ...
            dcc.Dropdown(id='column-selector', multi=True, placeholder="Select columns", value=["close"], className="ui dropdown",
                         style={"borderRadius": "15px", "backgroundColor": "transparent", "boxShadow": "none"}),
            dcc.Store(id='data-store', data=None)
        ], className="ui raised segment", style={"borderRadius": "15px", "backgroundColor": f"{custom_blue_colors[1]}10", "color": custom_blue_colors[2]})
    elif tab == 'news-tab':
        return html.Div([
            html.H3("Latest News Headlines", className="ui header", style={"color": custom_blue_colors[2]}),
            html.Ul([html.Li("News item 1", style={"color": custom_blue_colors[2]}), html.Li("News item 2", style={"color": custom_blue_colors[2]}), html.Li("News item 3", style={"color": custom_blue_colors[2]})], className="ui list")
        ], className="ui raised segment", style={"borderRadius": "15px", "backgroundColor": neutral_bg_color})
